...
def fetch_disposed_stocks() -> dict:
    ...
    try:
        ...
        if disp_df is not None and not disp_df.empty:
            ...
            # FinLab 的 disposal_information 格式依版本而異，嘗試解析
            if '處置結束日期' in disp_df.columns:
                active = disp_df[disp_df['處置結束日期'] >= today_str]
                for _, row in active.iterrows():
                    sid = str(row.get('股票代號', row.get('stock_id', ''))).strip()
                    if sid and sid not in disposed:
                        start = str(row.get('處置開始日期', '')).strip()
                        end   = str(row.get('處置結束日期', '')).strip()
                        disposed[sid] = {
                            "name": str(row.get('股票名稱', sid)),
                            "period": f"{start} ~ {end}",
                            "measures": "",
                            "source": "FinLab"
                        }
