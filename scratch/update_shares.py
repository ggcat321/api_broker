...
try:
    ...
    tse_divisor = tse_mcap / taiex_points if taiex_points else 1
    otc_divisor = otc_mcap / tpex_points if tpex_points else 1
    
    print(f"TAIEX Points: {taiex_points:.2f}, TSE Mcap: {tse_mcap:,.0f}, TSE Divisor: {tse_divisor:,.0f}")
    print(f"TPEX Points: {tpex_points:.2f}, OTC Mcap: {otc_mcap:,.0f}, OTC Divisor: {otc_divisor:,.0f}")
    ...
    print("Saved to static/shares.json with divisors and market types.")
